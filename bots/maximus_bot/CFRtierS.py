...
# ------------------ PREFLOP APPROX EQUITY TABLE (VERY COARSE) ------------------

# keyed by sorted ranks (e.g. ('A','K'), suited flag)
PREFLOP_TABLE = {
    (("A", "A"), False): 0.85,
    (("K", "K"), False): 0.82,
    (("Q", "Q"), False): 0.80,
...
def preflop_equity_approx(hole, n_opps):
    ...
    # more opponents → reduce equity a bit
...
def decide(state: dict) -> dict:
    try:
        ...
        # HYBRID EQUITY: preflop shortcut, light flop, full MC later
        if street == "preflop":
            equity = preflop_equity_approx(your_cards, n_opponents)
        elif street == "flop":
            # quick, cheap MC on flop
